...
/// copy. Mirrors the server's `distinct_windows`.
fn distinct_windows(
    items: &[FeedItem],
    now: DateTime<Utc>,
    limit: u32,
...
/// and shells alike). One `search_by_date` query for the whole week (complete,
/// newest-first), split into distinct tabs. Mirrors the server.
